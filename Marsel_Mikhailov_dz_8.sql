...
-- Домашнее Задание 1
SELECT 
	m.from_user_id AS 'id_Друга которого мы ищем',
	concat(u.firstname, '_', u.lastname) AS 'Наш друг', 
	COUNT(*) AS cnt
FROM messages m
...
-- Задание 2.
SELECT COUNT(m.id) AS 'Количество лайков'
FROM likes l
JOIN media m ON m.id =l.media_id 
JOIN profiles p ON m.id=p.user_id  
