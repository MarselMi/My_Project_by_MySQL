...
-- Создаю базу по музыке пользователя, связь 1-М
...
CREATE TABLE `music`(
	...
	FOREIGN KEY (music_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
-- Создаю связь M-M 
-- Таблица хранящая комментарии к медиа пользователя данные 
...
-- связь 1-1 не придумал как реализовать, все простые примеры уже приведены в примере, мне кажется

/* 
 * Пока еще не могу определиться с темой, думаю, может быть создать базу данных какого либо склада...
 * но конкретно какого не знаю
 */
