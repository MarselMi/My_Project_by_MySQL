...
SELECT 
	(SELECT id FROM users WHERE id=messages.from_user_id) AS 'id_Друга которого мы ищем',
	COUNT(*) AS cnt
FROM messages
...
-- Задание 2.
-- Нахожу пользователей  младше 11 лет
SELECT user_id AS 'id Пользователей младше 11 лет' 
FROM profiles
WHERE TIMESTAMPDIFF(year, birthday, NOW()) < 11;

-- В таблице медиа нахожу id медиафайлов, созданные пользователями младше 11 лет
...
-- Делаю общий запрос с таблицы лайков
SELECT COUNT(id)
FROM likes
WHERE media_id IN 
(SELECT id AS media_id 
...
-- СВЕРХУ ЧЕРНОВИКИ, ХОД МЫСЛЕЙ... НИЖЕ РЕШЕНИЕ ОБЬЕДИНЕНИЕМ

-- ЧЕРНОВИК. В одиночной выборке не составило труда
SELECT COUNT(user_id) AS WOMAN 
FROM likes
WHERE user_id IN (SELECT user_id FROM profiles WHERE gender='f');

-- ЧЕРНОВИК. Но вот совмещение оказалось труднее чем я ожидал
SELECT 
(SELECT COUNT(user_id) FROM likes WHERE gender='f') AS WOMAN,
(SELECT COUNT(user_id) FROM likes WHERE gender='m') AS MAN
FROM profiles
GROUP BY gender;

-- НЕВЕРНОЕ
SELECT COUNT(id) AS SUM_of_Likes, 
(SELECT COUNT(user_id) FROM profiles WHERE gender='m' AND user_id IN (SELECT user_id FROM likes)) AS MEN, 
(SELECT COUNT(user_id) FROM profiles WHERE gender='f' AND user_id IN (SELECT (user_id) FROM likes)) AS WOMAN 
FROM likes;

-- РЕЗУЛЬТАТ. Очень долго мудрил, не получалось в итоге пришел к такому решению. 
-- ВЫШЕ оставил варианты черновика
SELECT 
SUM(user_id IN (SELECT user_id FROM profiles WHERE gender='f')) AS WOMAN,
SUM(user_id IN (SELECT user_id FROM profiles WHERE gender='m')) AS MEN
FROM likes;
